...
function cookie_keyvalues_set(original_cookies, key, value) {
    ...
    if (selected_cookie_index == -1 && value != undefined) cookies_ar.push(new_element);
        ...
        if (value === undefined) cookies_ar.splice(selected_cookie_index, 1);
        else cookies_ar.splice(selected_cookie_index, 1, new_element);
    }
    ...
    for (let to_modify of config.headers) {
        if (
            ...
            if (to_modify.action === 'add') {
                ...
                if (header_cookie === undefined) {
                    if (config.debug_mode)
                        log('cookie_delete.req: no cookie header found. doing nothing for url ' + e.url);
                } else {
                    header_cookie.value = new_cookie;
                    if (config.debug_mode)
    ...
    for (let to_modify of config.headers) {
        if (
            ...
            if (to_modify.action === 'add') {
                    ...
                    if (config.debug_mode)
                        log(
                            'Delete response header :  name=' +
                                to_modify.header_name.toLowerCase() +
                                ' for url ' +
                                e.url
                        );
                }
                ...
                if (header_cookie === undefined) {
                    log(
                        "ModfyHeaders.Warning: you're using cookie_add_or_modify in Response. While adding new cookie in response, this plugin only generates `Set-Cookie: cookie-name=cookie-value `, without ANY additional attributes. Add a `Set-Cookie` header if you need them. "
                    );
                ...
                let index = e.responseHeaders.findIndex(
                    (header) =>
                        header.name.toLowerCase() === 'set-cookie' &&
                        header.value
                            .toLowerCase()
                ...
                if (index === -1) {
                    if (config.debug_mode)
                        log('cookie_delete.resp: no matching set-cookie header. doing nothing for url ' + e.url);
                } else {
                    e.responseHeaders.splice(index, 1);
                    if (config.debug_mode)
                        log('cookie_delete.resp delete_header : name=' + to_modify.header_name + ' for url ' + e.url);
                }
            }
        }
    }
